...
namespace BP_ZalohovaciNastroj
{
    public partial class Recovery : Form
    {
        ...
        {
            if (!Directory.Exists(TB_DestinationPath.Text))
            {
            ...
            foreach (FileInfo file in filesToRecovery)
            {
                if (file != null)
                {
                    ...
                    string temp2 = file.FullName;
                    string temp3 = file.FullName;
                    string test = temp.Replace(TB_RootPath.Text, TB_DestinationPath.Text).Substring(0, temp.Replace(TB_RootPath.Text, TB_DestinationPath.Text).LastIndexOf('\\') + 1) + originalNamesOfFiles[index];
                    if (!File.Exists(test))
                    {
                        try
                        {
                ...
                if (copyFileDialog.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    foreach (var item in files)
                    {
                        if (item.Value == true)
                        {
                            try
                            {
                                string temp = item.Key.FullName;
                                File.Copy(item.Key.FullName, temp.Replace(TB_RootPath.Text, TB_DestinationPath.Text).Substring(0, temp.Replace(TB_RootPath.Text, TB_DestinationPath.Text).LastIndexOf('\\') + 1) + originalNamesOfFiles[index++], true);
