...
namespace BP_ZalohovaciNastroj
{
    ...
    public partial class Backup : Form
    {
        ...
        private void B_DestionationPathLoader_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
                project.DestinationPath = folderBrowserDialog1.SelectedPath;
        ...
        {
            TB_DestinationPath.Text = project.DestinationPath;
            TB_RootPath.Text = project.SourcePath;
            NUD_NumberOfBackups.Value = project.NumberOfBackups;
            TV_FiltersView.Nodes.Clear();
            FillFilters(project.MainFilter);
        }
        private void FillFilters(AggregationFilter filter) 
        {
            int actualIndex = 0;
            foreach (var node in filter.Filters)
            {
                TV_FiltersView.Nodes.Add(BuildNodeForFilter(node, GetNegateForFilter(), GetNameForFilter()));
                if (node is AggregationFilter) 
                {
                    AggregationFilter newFilter = (AggregationFilter)node;
                    if (newFilter.Filters.Count > 0)
                        AddSubNode(newFilter, actualIndex);
                }
                actualIndex++;
            }
        }
        private void AddSubNode(AggregationFilter aggregationFilter, int actualIndex) 
        {
            foreach (var subnode in aggregationFilter.Filters)
            {
                TV_FiltersView.Nodes[actualIndex].Nodes.Add(BuildNodeForFilter(subnode, GetNegateForFilter(), GetNameForFilter()));
                if (subnode is AggregationFilter)
                {
                    AggregationFilter newFilter = (AggregationFilter)subnode;
                    if (newFilter.Filters.Count > 0)
                        AddSubNode(newFilter, actualIndex);
                }
            }
        }
        private void NUD_NumberOfBackups_ValueChanged(object sender, EventArgs e)
        {
            project.NumberOfBackups = (int)NUD_NumberOfBackups.Value;
        }

        private void TB_RootPath_TextChanged(object sender, EventArgs e)
        {
            project.SourcePath = TB_RootPath.Text;
        }

        private void TB_DestinationPath_TextChanged(object sender, EventArgs e)
        {
            project.DestinationPath = TB_DestinationPath.Text;
        }
